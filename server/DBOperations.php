...
public function __construct() {
 ...
 public function insertData($name,$mobile,$address,$email,$password){
	...
	$salt = $hash["salt"];
 	...
 	$sql = 'INSERT INTO users SET unique_id =:unique_id,name =:name,mobile =:mobile,address =:address,
    email =:email,encrypted_password =:encrypted_password,salt =:salt,created_at = NOW()';

 	$query = $this ->conn ->prepare($sql);
 	$query->execute(array('unique_id' => $unique_id, ':name' => $name, 'mobile' => $mobile, 
        'address' => $address, ':email' => $email, ':encrypted_password' => $encrypted_password, ':salt' => $salt));
